...
class ObjectDetection3D(Node):
    def __init__(self):
        ...
        self.get_logger().info('📡 Waiting for synchronized detection and depth data...')
    
    def transform_coordinates(self, yolo_x, yolo_y, yolo_width, yolo_height, depth_width, depth_height):
        """Transform from YOLOv8 coordinates to depth image coordinates"""
        # Original color camera resolution 
        color_width = 640
        color_height = 480
        
        # Scale from YOLOv8 (640x640) to color (640x480)
        ...
        # Scale from color (640x480) to depth (848x480)
        depth_x = color_x * (depth_width / color_width)
        depth_y = color_y  # Height is the same
    ...
    def process_detections(self, detection_msg, depth_msg, depth_info_msg, yolo_info_msg):
        """Process synchronized detection and depth data"""
        try:
            # Convert depth image
            ...
            self.get_logger().info(f'📐 Image sizes: YOLOv8={yolo_width}x{yolo_height}, Depth={depth_width}x{depth_height}')
            ...
            for i, detection in enumerate(detection_msg.detections):
                ...
                depth_x, depth_y = self.transform_coordinates(
                    bbox_x, bbox_y, yolo_width, yolo_height, depth_width, depth_height
                )
                ...
                if 0 <= depth_x < depth_width and 0 <= depth_y < depth_height:
                    ...
                    self.get_logger().warn(
                        f'⚠️  {class_name}: Coordinates ({depth_x},{depth_y}) outside depth image bounds'
                    )
